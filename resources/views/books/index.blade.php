...
<div class="container mt-4 mb-4">
    <div class="row g-3">
        ...
        <div class="{{ $promoCarousels->isNotEmpty() ? 'col-lg-8' : 'col-12' }}">
            ...
            <div id="katalogCarousel" class="carousel slide h-100" data-bs-ride="carousel" data-bs-interval="4000" style="border-radius: 20px; overflow: hidden; box-shadow: 0 5px 15px rgba(0,0,0,0.08);">
                @if($mainCarousels->count() > 1)
                <div class="carousel-indicators">
                    @foreach($mainCarousels as $banner)
                        <button type="button" data-bs-target="#katalogCarousel" data-bs-slide-to="{{ $loop->index }}" class="{{ $loop->first ? 'active' : '' }}"></button>
                    @endforeach
                </div>
                @endif
                        ...
                        <div class="carousel-item h-100 {{ $loop->first ? 'active' : '' }}">
                            ...
                            <div class="carousel-caption d-none d-md-block" style="z-index: 2; bottom: 20px; text-shadow: 0 2px 4px rgba(0,0,0,0.8);">
                                ...
                                @if($banner->description)
                                    <p class="mb-0 small text-white">{{ $banner->description }}</p>
                                @endif
                            </div>
                        </div>
                    @endforeach
                ...
                @if($mainCarousels->count() > 1)
                <button class="carousel-control-prev" type="button" data-bs-target="#katalogCarousel" data-bs-slide="prev">
                    <span class="carousel-control-prev-icon"></span>
                </button>
                <button class="carousel-control-next" type="button" data-bs-target="#katalogCarousel" data-bs-slide="next">
        ...
        {{-- Promo Carousels (kanan, 2 banner ditumpuk) --}}
            ...
            <form action="{{ route('books.index') }}" method="GET" class="row g-2 align-items-center">
                <div class="col-md-6">
                    <div class="input-group">
                        ...
                        <input type="text" name="search" class="form-control border-start-0 ps-0"
                            placeholder="Cari buku berdasarkan judul atau penulis..." value="{{ request('search') }}"
                ...
                @auth
                    <div class="col-md-2 text-end">
                        <a href="{{ route('orders.index') }}" class="btn w-100 text-white"
                ...
                @foreach($books as $book)
                    <div class="col-6 col-md-3">
                        <div class="book-card card h-100 border-0 shadow-sm" style="border-radius:15px; overflow:hidden;">
                            <a href="{{ $route = route('books.show', $book) }}" class="position-relative d-block"
                                style="background-color: #f1f5f9; padding: 1rem;">
                                ...
                                @if($book->stock > 0)
                                    <span class="badge position-absolute"
                                        style="top: 10px; right: 10px; background-color: #10b981; padding: 6px 12px; font-weight: 600;">Tersedia</span>
                                @else
                                    <span class="badge position-absolute bg-danger"
                                        style="top: 10px; right: 10px; padding: 6px 12px; font-weight: 600;">Habis</span>
                                @endif
                            </a>
                            <div class="card-body p-3 d-flex flex-column">
                                <p class="text-muted small mb-1" style="font-size: 0.75rem;"><i
                                        class="bi bi-tag-fill me-1"></i>{{ $book->category->name }}</p>
                                ...
                                <div class="mt-auto pt-2 border-top">
                                    <span class="fw-bold"
                                        style="color: #4f46e5; font-size:1.1rem;">{{ $book->formatted_price }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
